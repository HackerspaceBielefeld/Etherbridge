...
uint8_t checkRequest(uint8_t inBuffer[], uint16_t msgLength, const uint32_t remoteIP, const uint16_t remotePort, uint8_t requestType)
{
	...
	if (queueData.size() > queueDataSize)
	{
		queueDataSize = queueData.size();
	}

	if (queueHeaders.size() > queueHeadersSize)
	{
...
void setSlaveStatus(const uint8_t slave, uint8_t status, const bool value, const bool isScan)
{
	...
	else
	{
		...
		slaveStatus[status][slave / 8] |= masks[slave & 7];
		if (status != SLAVE_ERROR_0B_QUEUE && isScan == false)
		{
			data.errorCnt[status]++;  // there is no counter for SLAVE_ERROR_0B_QUEUE, ignor scans in statistics
		}
	}
}
	...
	for (uint8_t s = 0; s < maxSockNum; s++) // for each hardware socket ...
	{
		...
		switch (status)
		{
			...
			case SnSR::LAST_ACK:
			{
				...
				if (sockAge > TCP_DISCON_TIMEOUT)
				{  //     if it's been more than TCP_CLIENT_DISCON_TIMEOUT since disconnect command was sent...
...
void sendResponse(const uint8_t MBAP[], const uint8_t PDU[], const uint16_t pduLength)
{
	...
	else if (myHeader.requestType & TCP_REQUEST)
	{
		...
		if (W5100.readSnSR(sock) == SnSR::ESTABLISHED && W5100.readSnDPORT(sock) == myHeader.remPort)
		{  // Check remote port should be enough or check also rem IP?
			if (data.config.enableRtuOverTcp)
			{
