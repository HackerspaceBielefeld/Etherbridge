...
static void delay(uint32_t ms)
{
    ...
    while(SysTick_GetMillis() - start >= ms);
}
...
uint16_t W5100Class::write(uint16_t addr, const uint8_t *buf, uint16_t len)
{
	...
	if (len <= 5) {
		...
		while(wzChannel.shift(cmd, len + 3) != SUCCESS);
	} else {
		while(wzChannel.shift(cmd, 3) != SUCCESS);
#ifdef SPI_HAS_TRANSFER_BUF
			SPI.transfer(buf, NULL, len);
#else
		// TODO: copy 8 bytes at a time to cmd[] and block transfer
		for (uint16_t i=0; i < len; i++) {
		    while(wzChannel.write(buf[i]) != SUCCESS);
		}
#endif
	}
	while(wzChannel.end() != SUCCESS);
...
uint16_t W5100Class::read(uint16_t addr, uint8_t *buf, uint16_t len)
{
	...
	while(wzChannel.shift(cmd, 3) != SUCCESS);
	memset(buf, 0, len);
	while(wzChannel.shift(buf, len) != SUCCESS);
	while(wzChannel.end() != SUCCESS);
