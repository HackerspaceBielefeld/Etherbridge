...
int DNSClient::getHostByName(const char* aHostname, IPAddress& aResult, uint16_t timeout)
{
	...
	if (iUdp.begin(1024+(SysTick_GetMillis() & 0xF)) == 1) {
...
uint16_t DNSClient::BuildRequest(const char* aName)
{
	...
	// some of this header
	iRequestId = SysTick_GetMillis(); // generate a random ID
...
{
	uint32_t startTime = SysTick_GetMillis();

	// Wait for a response packet
	while (iUdp.parsePacket() <= 0) {
		if ((SysTick_GetMillis() - startTime) > aTimeout) {
			return TIMED_OUT;
		}
//		delay(50);
	}
