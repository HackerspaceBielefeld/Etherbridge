...
int DhcpClass::request_DHCP_lease()
{
	...
	//Random number 1 - 2000
	_dhcpTransactionId = (SysTick_GetRandom() % 2000) + 1;
	...
	unsigned long startTime = SysTick_GetMillis();

	while (_dhcp_state != STATE_DHCP_LEASED) {
		if (_dhcp_state == STATE_DHCP_START) {
			_dhcpTransactionId++;
			send_DHCP_MESSAGE(DHCP_DISCOVER, ((SysTick_GetMillis() - startTime) / 1000));
			_dhcp_state = STATE_DHCP_DISCOVER;
		} else if (_dhcp_state == STATE_DHCP_REREQUEST) {
			_dhcpTransactionId++;
			send_DHCP_MESSAGE(DHCP_REQUEST, ((SysTick_GetMillis()- startTime)/1000));
		...
		if (result != 1 && ((SysTick_GetMillis() - startTime) > _timeout))
			break;
	}
	...
	_lastCheckLeaseMillis = SysTick_GetMillis();
	return result;
}
...
uint8_t DhcpClass::parseDHCPResponse(unsigned long responseTimeout, uint32_t& transactionId)
{
	...
	while (_dhcpUdpSocket.parsePacket() <= 0) {
		if ((SysTick_GetMillis() - startTime) > responseTimeout) {
			return 255;
		}
//		delay(50);
	}
...
int DhcpClass::checkLease()
{
	...
	unsigned long now = SysTick_GetMillis();
